/******/ (() => {
	...
	/******/ var __webpack_modules__ = {
		...
		/***/ 661: /***/ () => {
			...
			const changeButtons = document.querySelectorAll(
				".cabinet-hero__button-change"
			);
			...
			if (
				...
				categories.forEach((category) => {
					category.addEventListener("click", () => {
						...
						hiddenContainer.classList.remove(
							"catalog-hero__container-hidden"
						);
		...
		/***/ 774: /***/ () => {
			const moreBtn = document.querySelector(
				".main-feedback__button-more"
			);
			const feedbackSlides = document.querySelectorAll(
				".main-feedback__slide"
			);
			if (moreBtn !== null) {
	...
	(() => {
		...
		var radio_switch = __webpack_require__(848); // CONCATENATED MODULE: ./node_modules/ssr-window/ssr-window.esm.js
		/**
		 * SSR Window 4.0.2
		...
		function utils_getSlideTransformEl(slideEl) {
			return (
				...
				(slideEl.shadowEl &&
					slideEl.shadowEl.querySelector(
      ...
      'marginRight': 'marginBottom'
		...
		function loopFix({
			...
			if (loopedSlides % params.slidesPerGroup !== 0) {
				loopedSlides +=
					params.slidesPerGroup -
					(loopedSlides % params.slidesPerGroup);
			}
			...
			if (activeSlideIndex < loopedSlides) {
				...
				for (let i = 0; i < loopedSlides - activeSlideIndex; i += 1) {
					const index =
						i - Math.floor(i / slides.length) * slides.length;
    ...
    'watch-progress': params.watchSlidesProgress
				...
				Object.assign(swiper, {
					...
					rtlTranslate:
						...
						(el.dir.toLowerCase() === "rtl" ||
							elementStyle(el, "direction") === "rtl"),
					wrongRTL:
						elementStyle(wrapperEl, "display") === "-webkit-box",
				});
				return true;
			}
			init(el) {
		...
		function Virtual({ swiper, extendParams, on, emit }) {
			...
			function update(force) {
				...
				const { addSlidesBefore, addSlidesAfter } =
					swiper.params.virtual;
				const {
					from: previousFrom,
					to: previousTo,
				...
				if (centeredSlides) {
					...
					slidesBefore =
						(isLoop ? slidesPerView : slidesPerGroup) +
						addSlidesBefore;
				}
					...
					if (
						...
						swiper.slides.forEach((slideEl) => {
							slideEl.style[offsetProp] = `${
								offset -
								Math.abs(swiper.cssOverflowAdjustment())
							}px`;
						});
					}
					swiper.updateProgress();
				...
				if (
					...
					if (!ignoreWheelEvents) {
						...
						if (swiper.params.loop) {
							swiper.loopFix({
								direction:
									newEvent.direction < 0 ? "next" : "prev",
								byMousewheel: true,
							});
						}
		...
		function Pagination({ swiper, extendParams, on, emit }) {
			...
			function onBulletClick(e) {
				...
				if (swiper.params.loop) {
					...
					if (
						newSlideIndex >
						swiper.slides.length - swiper.loopedSlides
					) {
						swiper.loopFix({
							direction:
			...
			function init() {
				...
				if (
					...
					if (el.length > 1) {
						el = el.filter((subEl) => {
							if (
								utils_elementParents(subEl, ".swiper")[0] !==
								swiper.el
							)
								return false;
							return true;
		...
		function Zoom({ swiper, extendParams, on, emit }) {
			...
			function eventWithinZoomContainer(e) {
				...
				if (
					[...swiper.el.querySelectorAll(selector)].filter(
			...
			function zoomIn(e) {
				...
				if (e && !(currentScale === 1 && forceZoomRatio)) {
					...
					offsetX =
						elementOffset(gesture.slideEl).left + window.scrollX;
					offsetY =
			...
			const getSlide = (el) => {
				if (!el.parentElement) {
					...
					const slide = swiper.slides.filter(
						(slideEl) =>
							slideEl.shadowEl &&
							slideEl.shadowEl === el.parentNode
					)[0];
					return slide;
				}
					...
					if (isHorizontal) {
						...
						const multiplier =
							...
							(Math.sin((shadowAngle * 2 * Math.PI) / 360) / 2 +
								Math.cos((shadowAngle * 2 * Math.PI) / 360) /
									2);
				...
				for (let i = 0; i < slides.length; i += 1) {
					...
					if ((custom && data.shadow) || !custom) {
						...
						if (!shadowEl && data.shadow) {
							shadowEl = create_shadow_createShadow(
								params,
								slideEl
							);
						}
      ...
      `;
		...
		const swiperHero = new core(
			document.querySelector(".main-hero__slider"),
			{
				slidesPerView: "auto",
				spaceBetween: 40,
				resistanceRatio: 0,
		...
		const quizInnerSlider = new core(".quiz__slider-inner", {
			slidesPerView: 1,
			spaceBetween: 15,
			speed: 500,
			allowSlideNext: false,
		...
		const designSlider = new core(".main-design__slider", {
			slidesPerView: 1,
			speed: 500,
			allowSlideNext: false,
			allowSlidePrev: false,
		});
		const slideToHide = document.querySelector(
			".main-design__slide-tohide"
		);
		const designNextBtn = document.querySelector(
			".main-design__button-next"
		);
		...
		const designPrevBtns = document.querySelectorAll(
			".main-design__button-prev"
		);
		...
		// EXTERNAL MODULE: ./src/js/components/maps.js
		var maps = __webpack_require__(27); // CONCATENATED MODULE: ./src/js/components/tabs.js
		class Tabs {
			constructor(selector, innerElems, starterId) {
				...
				if (this.tabs) {
					this.tabList = this.tabs.querySelector(
						"." + innerElems.list
					);
			...
			events() {
				this.tabsBtns.forEach((el, i) => {
					el.addEventListener("click", (e) => {
						let currentTab =
							this.tabList.querySelector("[aria-selected]");
			...
			switchTabs(newTab) {
				...
				let oldIndex = Array.prototype.indexOf.call(
					this.tabsBtns,
					oldTab
				);
		...
		// EXTERNAL MODULE: ./src/js/components/cabinet.js
		var cabinet = __webpack_require__(661);
		// EXTERNAL MODULE: ./src/js/components/menu.js
		var menu = __webpack_require__(617);
		// EXTERNAL MODULE: ./src/js/components/video-controller.js
		var video_controller = __webpack_require__(578);
		// EXTERNAL MODULE: ./src/js/components/more-btn.js
		var more_btn = __webpack_require__(774);
		// EXTERNAL MODULE: ./src/js/components/categories.js
		var categories = __webpack_require__(472);
		// EXTERNAL MODULE: ./src/js/components/catalog-filters.js
		var catalog_filters = __webpack_require__(586); // CONCATENATED MODULE: ./src/js/functions/mobile-check.js
		const mobileCheck = () => {
			const userAgent =
				navigator.userAgent || navigator.vendor || window.opera;
			if (/android/i.test(userAgent)) {
				_vars.htmlEl.classList.add("page--android");
				return "Android";
			}
			if (/iPad|iPhone|iPod/.test(userAgent) && !window.MSStream) {
				_vars.htmlEl.classList.add("page--ios");
				return "iOS";
			}
			return "unknown";
		}; // CONCATENATED MODULE: ./src/js/components/search-icons.js
		...
		} // CONCATENATED MODULE: ./src/js/functions/disable-scroll.js
		const disableScroll = () => {
			const fixBlocks = document?.querySelectorAll(".fixed-block");
			const pagePosition = window.scrollY;
			const paddingOffset = `${
				window.innerWidth - _vars.bodyEl.offsetWidth
			}px`;
			_vars.htmlEl.style.scrollBehavior = "none";
			fixBlocks.forEach((el) => {
				el.style.paddingRight = paddingOffset;
			});
			_vars.bodyEl.style.paddingRight = paddingOffset;
			_vars.bodyEl.classList.add("dis-scroll");
			_vars.bodyEl.dataset.position = pagePosition;
			_vars.bodyEl.style.top = `-${pagePosition}px`;
		}; // CONCATENATED MODULE: ./src/js/functions/enable-scroll.js
		const enableScroll = () => {
			const fixBlocks = document?.querySelectorAll(".fixed-block");
			const body = document.body;
			const pagePosition = parseInt(_vars.bodyEl.dataset.position, 10);
			fixBlocks.forEach((el) => {
				el.style.paddingRight = "0px";
			});
			_vars.bodyEl.style.paddingRight = "0px";
			_vars.bodyEl.style.top = "auto";
			_vars.bodyEl.classList.remove("dis-scroll");
			window.scroll({
				top: pagePosition,
				left: 0,
			});
			_vars.bodyEl.removeAttribute("data-position");
			_vars.htmlEl.style.scrollBehavior = "smooth";
		}; // CONCATENATED MODULE: ./src/js/components/forms.js
		const formSection = document.querySelector(".forms");

		if (formSection !== null) {
			const formsContainer =
				formSection.querySelector(".forms__container");
			const header = document.querySelector(".header");
			...
			const closeBtns = formSection.querySelectorAll(
				".forms__button-close"
			);
			...
			const emailEnterButton =
				formSection.querySelector(".forms__item-email");
			const emailRegContainer =
				document.getElementById("emailRegContainer");
			emailEnterButton.addEventListener("click", () => {
				formsContainersInner.forEach((container) => {
					container.classList.add("forms__container-inner-hidden");
				});
				emailRegContainer.classList.remove(
					"forms__container-inner-hidden"
				);
			});

			// форма входа по логину
			const loginFormsBtns =
				formSection.querySelectorAll(".login-form-btn");
			...
			const recoverFormsBtn = formSection.querySelector(
				".forms__button-recover"
			);
			...
			recoverFormsBtn.addEventListener("click", () => {
				...
				recoverFormContainer.classList.remove(
					"forms__container-inner-hidden"
				);
			});
			...
			const regFormsBtns =
				formSection.querySelectorAll(".forms__button-reg");
			...
			const mainFormBackBtns = document.querySelectorAll(
				".main-form-fack-btn"
			);
			const mainFormContainer = document.getElementById(
				"main-form-container"
			);
			mainFormBackBtns.forEach((btn) => {
				btn.addEventListener("click", () => {
					...
					mainFormContainer.classList.remove(
						"forms__container-inner-hidden"
					);
				});
			});
			...
			const loginFormBackBtn = document.getElementById(
				"login-form-back-btn"
			);
			...
			const recoverFormBackBtn = document.getElementById(
				"recover-form-back-btn"
			);
		...
		// EXTERNAL MODULE: ./src/js/components/up-button.js
		var up_button = __webpack_require__(290); // CONCATENATED MODULE: ./src/js/_components.js // CONCATENATED MODULE: ./src/js/main.js
	})();
