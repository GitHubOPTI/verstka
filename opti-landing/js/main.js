...
*/
  ...
  */
		...
		var gsapWithCSS = gsap.registerPlugin(CSSPlugin) || gsap,
			...
			TweenMaxWithCSS = gsapWithCSS.core.Tween; // CONCATENATED MODULE: ./node_modules/gsap/utils/paths.js
 ...
 */
		...
		function getRawPath(value) {
			...
			return !value
				...
				: paths_isString(value)
				? stringToRawPath(value)
				: paths_isNumber(value[0])
				? [value]
				: value;
		...
		function subdivideSegment(segment, i, t) {
			...
			segment.splice(
				i + 2,
				4,
				paths_round(x1a), //first control point
				paths_round(y1a),
				paths_round(x1), //second control point
				paths_round(y1),
				paths_round(x1 + (x2 - x1) * t), //new fabricated anchor on line
				paths_round(y1 + (y2 - y1) * t),
				paths_round(x2), //third control point
				paths_round(y2),
				paths_round(x2a), //fourth control point
				paths_round(y2a)
			);
		...
		function arcToSegment(lastX, lastY, rx, ry, angle, largeArcFlag, sweepFlag, x, y) {
			...
			var coef = (largeArcFlag === sweepFlag ? -1 : 1) * paths_sqrt(sq),
				...
				angleExtent =
					(ux * vy - uy * vx < 0 ? -1 : 1) *
					Math.acos((ux * vx + uy * vy) / paths_sqrt(temp * (vx * vx + vy * vy)));
...
*/
		...
		function pointsToSegment(points, curviness) {
			...
			for (i = 2; i < l; i += 2) {
				...
				if (x !== prevX || y !== prevY) {
					segment.push(
						paths_round(mx1 + mxm), // first control point
						paths_round(my1 + mym),
						paths_round(x), // anchor
						paths_round(y),
						paths_round(mx2 + mxm), // second control point
						paths_round(my2 + mym)
					);
				}
			}
...
*/
		...
		var matrix_doc,
			...
			_getDocScrollTop = function _getDocScrollTop() {
				return (
					matrix_win.pageYOffset ||
					matrix_doc.scrollTop ||
					matrix_docElement.scrollTop ||
					_body.scrollTop ||
					0
				);
			},
			_getDocScrollLeft = function _getDocScrollLeft() {
				return (
					matrix_win.pageXOffset ||
					matrix_doc.scrollLeft ||
					matrix_docElement.scrollLeft ||
					_body.scrollLeft ||
					0
				);
			},
			...
			_createSibling = function _createSibling(element, i) {
				if (element.parentNode && (matrix_doc || _setDoc(element))) {
					var svg = _svgOwner(element),
						...
						e = matrix_doc.createElementNS
							? matrix_doc.createElementNS(ns.replace(/^https/, "http"), type)
							: matrix_doc.createElement(type);
			...
			_getCTM = function _getCTM(svg) {
				...
				if (!m) {
					...
					transform
						? (svg.style[matrix_transformProp] = transform)
						: svg.style.removeProperty(matrix_transformProp.replace(/([A-Z])/g, "-$1").toLowerCase());
				}
		...
		var _xProps = "x,translateX,left,marginLeft,xPercent".split(","),
			...
			_getGSAP = function _getGSAP() {
				return (
					...
					(typeof window !== "undefined" &&
						(MotionPathPlugin_gsap = window.gsap) &&
						MotionPathPlugin_gsap.registerPlugin &&
						MotionPathPlugin_gsap)
				);
			},
			...
			_addDimensionalPropTween = function _addDimensionalPropTween(
				...
				var cache = target._gsap,
					...
					pt = (plugin._pt = new MotionPathPlugin_PropTween(
						plugin._pt,
						target,
						prop,
						0,
						0,
						MotionPathPlugin_emptyFunc,
						0,
				...
				if (data.tween._time || !MotionPathPlugin_reverting()) {
					...
					data.rotate &&
						data.rSet(
							data.target,
							data.rProp,
							rawPaths[0].angle * (data.radians ? MotionPathPlugin_DEG2RAD : 1) + data.rOffset + data.ru,
							data,
							ratio
						);
				} else {
		...
		_getGSAP() && MotionPathPlugin_gsap.registerPlugin(MotionPathPlugin); // CONCATENATED MODULE: ./src/js/components/hero-kit.js
		...
		if (!mediaQuery480.matches) {
			...
			setInterval(() => {
				...
				if (
					imgCoords.top - img.offsetHeight / 2 < btnCoords.top &&
					imgCoords.left < btnCoords.left + btn.offsetWidth
				) {
		...
		// EXTERNAL MODULE: ./src/js/components/view-buttons.js
		var view_buttons = __webpack_require__(393); // CONCATENATED MODULE: ./src/js/_components.js // CONCATENATED MODULE: ./src/js/main.js
	})();
